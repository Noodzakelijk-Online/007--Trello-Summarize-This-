...
It also installs a `Configure Trello Power-Up` shortcut. That shortcut opens a setup assistant that prepares the iframe connector URL, app metadata, icon URL, and capability list for Trello's Power-Up Admin Portal. The assistant can copy a readiness checklist or JSON setup package with exact admin values, validation state, safety notes, manual steps, and a no-submit autofill helper.
