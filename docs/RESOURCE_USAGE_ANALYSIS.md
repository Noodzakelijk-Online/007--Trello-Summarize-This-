...
- Active popup initial local files: about 336.5 KB (`popup.html`, `attachment-processor.js`, `summarizer-core.js`, `card-intelligence-ledger.js`, `icon.svg`).
- Windows installer runtime payload: about 499.2 KB.
- Whole repository source footprint, excluding `.git` and `dist`: about 1.77 MB.
- Generated Windows installer executable: 304,640 bytes.
...
2. Bounded AI response size and time:
   ...
   - The popup reuses the latest matching private ledger run when the stable card input hash and compact settings profile hash match, avoiding duplicate provider calls on repeated opens.
...
6. Lightweight ledger:
   ...
   - Cached run matching stores only a compact settings/profile hash plus bounded non-secret profile metadata; API keys and full proxy endpoints are not stored.
